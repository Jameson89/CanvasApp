  ...
  if ([hitView isKindOfClass:[KalTileView class]]) {
    ...
    if (tile.belongsToAdjacentMonth) {
      ...
      self.selectedTile = tile;
		...
		NSDate * date = [[[self.selectedTile.date NSDate] copy] autorelease];
		NSDictionary *info = [[NSDictionary alloc] initWithObjectsAndKeys:date, @"date", nil];
		[[NSNotificationCenter defaultCenter] postNotificationName:@"KalDataSourceChangedNotification" object:nil userInfo:info];
				NSLog(@"Yo7");
		[info release];

    }	
	//Insert way to re-load cal table info based on this
	//Maybe a post notification that a new date has been selected.
	 // NSLog(@"%@",self.selectedTile.date);
