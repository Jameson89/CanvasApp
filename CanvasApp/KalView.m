...
- (id)initWithFrame:(CGRect)frame delegate:(id<KalViewDelegate>)theDelegate logic:(KalLogic *)theLogic
{
  if ((self = [super initWithFrame:frame])) {
    ...
    [self addSubview:contentView];
	 ...
	 //pullDown = [[pullDownMenu alloc] initWithFrame:CGRectMake(0.f, -180.f, 320.f, 230.f)];
	//  NSArray* nibViews =  [[NSBundle bundleForClass:[self class]] loadNibNamed:@"pullDownMenu" owner:self options:nil];
	 // pullDown = [nibViews objectAtIndex:0];
	 // pullDown.frame = CGRectMake(0.f, -180.f, 320.f, 230.f);
...
- (void)receivedTouches:(NSSet *)touches withEvent:event
{
	...
	//CGPoint check = CGPointMake(location.x, location.y+5);
	...
	if (myView == nil) {
		...
		if ([hitView isKindOfClass:[pullDownMenu class]]) {
			myView = hitView;
			startPoint = location;
		}
	}
	if (location.y >= 0 && location.y <= 230) {
		myView.frame = CGRectMake(0, (location.y+15)-230, 320, 230);
	}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	//if ([hitView isKindOfClass:[pullDownMenu class]]) {
	//NSLog(@"%f", startPoint.y-location.y);
	if (startPoint.y-location.y > 20 ) {
		[UIView beginAnimations:nil context:NULL];
		[UIView setAnimationDuration:0.5];
		[UIView setAnimationDelegate:self];
		
		pullDown.frame = CGRectMake(0.f, -199.f, 320.f, 230.f);
		pullDown.dimmer.alpha = 0.0;
		
		[UIView commitAnimations];
	} else if (startPoint.y-location.y < -10) {
		[UIView beginAnimations:nil context:NULL];
		[UIView setAnimationDuration:0.5];
		[UIView setAnimationDelegate:self];
		
		pullDown.frame = CGRectMake(0.f, 0.f, 320.f, 230.f);
		pullDown.dimmer.alpha = 0.6;
		
		[UIView commitAnimations];
	} else if (myView.frame.origin.y <= -60) {
		[UIView beginAnimations:nil context:NULL];
		[UIView setAnimationDuration:0.5];
		[UIView setAnimationDelegate:self];
